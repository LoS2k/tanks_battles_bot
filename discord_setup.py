...
CATEGORIES = [
    ...
    # ── 🧪 TESTING — тільки Alpha + Beta Tester ──────────────────────────────
    {
        "name": "🧪 TESTING",
        "public": False,
        "tester_only": True,
        "channels": [
            {"name": "📋│tester-rules",    "type": "text",  "topic": "Testing rules & NDA — read before posting"},
            {"name": "💬│tester-chat",     "type": "text",  "topic": "Private chat for testers only"},
            {"name": "🐛│bugs-private",    "type": "forum", "topic": "Private bug reports — create a post per bug",
             "tags": ["🔴 Critical", "🟡 Major", "🟢 Minor", "✅ Fixed", "🔁 Duplicate"]},
            {"name": "⭐│feedback",        "type": "forum", "topic": "Game feedback — one post per feature",
             "tags": ["🎮 Gameplay", "🎨 Graphics", "🔊 Audio", "📱 Mobile", "💡 Idea"]},
            {"name": "🆕│feature-requests","type": "forum", "topic": "Feature requests — one idea per post",
             "tags": ["🔥 Hot", "💡 New", "🔄 In Progress", "✅ Planned", "❌ Declined"]},
            {"name": "📊│dev-polls",       "type": "text",  "topic": "Vote on upcoming features"},
            {"name": "🔊 Tester Voice",    "type": "voice"},
        ]
    },

    # ── 🌍 COMMUNITY — всі гравці, форум ─────────────────────────────────────
    {
        "name": "🌍 COMMUNITY",
        "public": True,
        "channels": [
            {"name": "🐛│bug-reports",  "type": "forum", "topic": "Report a bug — one post per bug",
             "tags": ["🔴 Critical", "🟡 Major", "🟢 Minor", "✅ Fixed", "🔁 Duplicate"]},
            {"name": "💡│suggestions",  "type": "forum", "topic": "Game suggestions — one idea per post",
             "tags": ["🔥 Popular", "💡 New", "✅ Planned", "❌ Declined"]},
            {"name": "⭐│reviews",      "type": "forum", "topic": "Share your game review",
             "tags": ["⭐⭐⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐", "⭐⭐", "⭐"]},
            {"name": "❓│questions",    "type": "text",  "topic": "Ask anything about the game"},
            {"name": "🎨│fan-art",      "type": "text",  "topic": "Share your fan art and creations"},
        ]
    },
...
async def run_setup(guild: discord.Guild):
    ...
    for cat_data in CATEGORIES:
        ...
        is_tester_only = cat_data.get("tester_only", False)

        # overwrites категорії
        ow: dict = {
            guild.default_role: discord.PermissionOverwrite(
                read_messages=is_public and not is_dev and not is_staff and not is_tester_only
            )
        }

        # Обмежені категорії: Dev / Staff
        ...
        # Tester-only: тільки Alpha + Beta Tester бачать і пишуть
        if is_tester_only:
            for rname in ("🧪 Alpha Tester", "🔬 Beta Tester"):
                r = discord.utils.get(guild.roles, name=rname)
                if r:
                    ow[r] = discord.PermissionOverwrite(read_messages=True, send_messages=True)

        for r in admin_roles:   # адміни завжди бачать усе
            ow[r] = discord.PermissionOverwrite(read_messages=True, send_messages=True)
        ...
        for ch in cat_data.get("channels", []):
            if ch["type"] == "voice":
                ...
                forum_ow = {
                    guild.default_role: discord.PermissionOverwrite(
                        read_messages=ow.get(guild.default_role,
                            discord.PermissionOverwrite()).read_messages,
                        send_messages=False,
                        create_public_threads=True,
                        send_messages_in_threads=True
                    )
                }
                for role, perms in ow.items():
                    if role != guild.default_role:
                        forum_ow[role] = discord.PermissionOverwrite(
                ...
                try:
                    forum_ch = await guild.create_forum_channel(
                        ch["name"], category=category,
                        topic=ch.get("topic", ""), overwrites=forum_ow
                    )
                    if ch.get("tags"):
                        tags = [discord.ForumTag(name=t) for t in ch["tags"]]
                        await forum_ch.edit(available_tags=tags)
                except AttributeError:
                    topic = ch.get("topic", "") + " | 📌 Один баг/пропозиція = один тред"
                    await guild.create_text_channel(
                        ch["name"], category=category,
                        topic=topic, overwrites=forum_ow
                    )
                    print(f"      ⚠️  Forum not supported, created as text: #{ch['name']}")

            else:
                # Звичайний текстовий канал
